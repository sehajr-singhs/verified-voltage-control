...
A large AI training cluster is a load that steps its active power by tens of percent every few seconds, synchronously across thousands of GPUs, because the training loop alternates between a compute phase that pulls full power and a communication phase that idles the accelerators while gradients sync. That square wave lands on a distribution feeder sized for slowly varying load, so the bus voltages swing with it and cross the plus-or-minus 5 percent limit that every interconnection agreement is written against, which is a real part of why utilities are now slow to connect these campuses. The feeder physics is what sets the constraint, because on a radial line the voltage at each bus is an affine function of the active and reactive power injected along the path back to the substation, so a reactive source at the data-center bus can push the voltage back, and the control problem reduces to deciding how much reactive power to inject from the local voltage alone, fast enough to track a load that moves in seconds.

The paper I verify solves that with a decentralized droop whose voltage reference switches with the workload mode, and it proves the reference-tracking error contracts whenever the gain satisfies 0 < K < 2X^-1. What I add is the verification, and it splits cleanly in two because the published controller is piecewise affine, so per mode the closed loop is a linear system and its safety is a spectral radius and a matrix sum rather than a search, which I compute exactly on the real IEEE 33-bus feeder. Then I put a small neural controller in the same loop and certify it with CROWN bound propagation, which proves for every voltage state in an operating box and every data-center load across the full mode range, not a sampled subset of them, that one closed-loop step keeps every bus inside the band. The one idea the repo turns on is that an affine controller's guarantee is a closed-form calculation, and bound propagation is the tool that carries that same kind of guarantee to a nonlinear network over a whole region at once, which is the object an operator actually needs before a learned controller goes near a real feeder, because a sampled confidence score is not something you can put in an interconnection study but a proof over the operating envelope is.
...
- Prof. Cui's related public code, the closest ancestors of this certified-control line: [Wenqi-Cui/Lyapunov-Regularized-RL](https://github.com/Wenqi-Cui/Lyapunov-Regularized-RL) (forked to [sehajr-singhs/Lyapunov-Regularized-RL](https://github.com/sehajr-singhs/Lyapunov-Regularized-RL) as the upstream reference) and [Wenqi-Cui/MIMO-Neural-PI](https://github.com/Wenqi-Cui/MIMO-Neural-PI). No public code release exists for arXiv:2603.15588, and no `Wenqi-Cui/Voltage-Control` repo exists (checked July 2026), so this repo stands alone on the paper and forks her closest real repo as the lineage link rather than fabricating a fork of this paper. See `NOTES.md`.
